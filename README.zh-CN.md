...
这是一个简单的 Python 工具，可以从网页 HTML 中提取图片地址并将图片下载到本地。对于普通网站，程序默认优先使用页面提供的原图/高清地址（例如 `data-full`、`data-hires`、`srcset` 或图片链接），没有这些地址时才回退到 `src` 缩略图。对于 note.com 的图片，会模拟点击图片后使用的 CDN 高清参数（4000 像素范围、质量 90），并按同一资源路径只保留最高质量版本。图片 URL 的查询参数修改是可选的，默认不会修改原始链接。
...
开始下载前，程序会询问是否修改图片 URL 的查询参数。直接回车或选择 `n` 会保留原始图片链接；选择 `y` 后，可以输入 `caw=1920&format=webp` 这样的参数。已有同名参数会被替换，新参数会被追加。
